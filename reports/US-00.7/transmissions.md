...
| **Ce qui reste vrai et ne doit pas être tu** | 🔧 **RECTIFIÉ le 2026-07-28 (A-1)** — ~~Le **test négatif d'US-00.1 n'a JAMAIS été exécuté**~~ : **c'est FAUX**. Il **A été exécuté le 2026-07-25** — faux secret, branche jetable, **PR draft #2**, job `🔐 Secrets scan (gitleaks)` = **`failure`** (run `30155284206`), attesté par `reports/US-00.1/README.md` l. 16 et 53-54, `qa.md` l. 38, `code_review.md` l. 166. ⚠️ **Erreur relevée par l'arbitrage @PO seul** — invisible aux 2 audits de code, aux 2 audits de sécurité et à la QA. ⛔ Texte d'origine **barré, non supprimé**. **Ce qui RESTE vrai, et c'est le point utile** : la preuve fournie ici démontre que *les 4 contextes sont requis et bloquants*, **pas** que *`secrets-scan` rouge bloque un merge* — le refus observé porte sur un contexte **`expected`** (non encore rapporté), **pas** sur un contexte **rouge**. ⛔ On ne casse pas volontairement un gate pour obtenir un rouge |
