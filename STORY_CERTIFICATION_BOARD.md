...
| US-01.4 | Gestes sur la tuile : révélation de la description et retrait d'une échue (RF-06) | development_start | ✅ @PO | ✅ @Data | ✅ @UX | ⏳ | ⏳ | ⏳ | ⏳ | ⏳ | ⏳ |
...
- **Phase** : `business_alignment` → `technical_validation` *(la cellule suit le **dernier événement tracé** — ⛔ défaut ③ qu'US-01.2 a payé : une phase affichée en avance ou en retard sur la trace est invisible des deux contrôles, chacun rendant « conforme » de son côté)*.
- **Track** : **FULL**, ⚖️ **rectifié le 2026-08-21 quelques heures après avoir été fixé à STANDARD**. **Motif en un seul exemplaire** dans le Story File. En deux mots : ADR-008 n'attache la correspondance scénario ↔ test **vérifiée par machine** qu'aux `.feature` de track FULL, donc en STANDARD les **37 scénarios de cette US n'auraient été sous AUCUN contrôle**, gate **vert**.
- **Périmètre** : ⚖️ **élargi par décision humaine du 2026-08-21** — US-01.4 ne portait que **RF-06** depuis l'arbitrage du 2026-08-03 ; elle fusionne désormais **① RF-06 inchangé** · **② la description révélée au simple tap, à la place du nombre, 3 s, sur une tuile `ACTIVE` seulement** · **③ nombre plus gros et centré**. ⛔ **Le motif d'origine du découpage n'est pas repeint** : il était vrai à sa date, et a perdu son objet quand US-01.2 a atteint `🧪 PASS`.
- ✅ **@PO Visa — 2026-08-21.** Story File **fonctionnel** *(AC, clauses, Gherkin, table anti-orphelin, critères de test fonctionnels, DoD)* + `tests/features/US-01.4-gestes-tuile.feature`. ⛔ **AUCUN COMPTE N'EST ÉCRIT ICI — j'y avais écrit `37 / 10 / 30` le 2026-08-21, et ces trois valeurs étaient FAUSSES le lendemain** *(AC-11 ajouté le 2026-08-22)*. ⛔ **Elles sont RETIRÉES, pas mises à jour** : une valeur mise à jour périme au cycle suivant — **démonstration en un jour**, défaut ⑤ commis dans le visa qui prétendait s'en garder. **Les commandes, qui sont l'autorité** : `grep -c "^  Scénario: " tests/features/US-01.4-gestes-tuile.feature` · `grep -c "^### AC-" docs/stories/US-01.4-gestes-tuile.md` · `grep -cE "^- \*\*(Nominal|Erreur|Limite)\*\* :" docs/stories/US-01.4-gestes-tuile.md` · doublons : `grep "^  Scénario: " … | sort | uniq -d` → **doit rendre une sortie VIDE**. **Le recoupement qui vaut plus que les nombres** : le compte de clauses doit égaler `AC × 3` ⇒ **aucun AC amputé d'une clause** *(seul contrôle capable de voir une clause **manquante** — la table anti-orphelin, elle, ne verrait que les clauses **écrites**)*, et la somme des scénarios **par AC** du tableau doit égaler le compte du `.feature` ⇒ ⛔ aucun scénario orphelin ni manquant. **Ces deux égalités se VÉRIFIENT, elles ne se citent pas.**
- ✅ **Gate *clarify* FERMÉ le 2026-08-21 — 10 arbitrages humains.** **Deux sont structurels** : **①** la révélation est **restreinte aux tuiles `ACTIVE`** et une tuile **`ÉCHUE` conserve sa description affichée en permanence** *(son « 0 » n'a rien à dire)* ⇒ **aucune tuile ne porte `onTap` ET `onDoubleTap`**, donc **pas de retard de `kDoubleTapTimeout` (300 ms)**, pas de clignotement, **risque nº 5 non aggravé**, et le scénario hérité d'US-01.2 *« aucun effet d'un appui simple ou prolongé »* reste **littéralement vrai** ⛔ **sans être amendé**. **②** L'**écart à WCAG 2.2.1 « Timing Adjustable » est DÉCLARÉ, pas nié** — 3 s + ré-appui illimité, et ⛔ *« re-déclenchable »* n'est **pas** une exception littérale de la norme. **@ProductOwner a refusé de conclure à la conformité** ; c'est le **premier registre d'écarts WCAG du projet**, alors que le design system affirme « WCAG AA » **sans en avoir un**.
- 📤 **ELLE AMENDE DEUX AC D'US-01.1, qui est à `🧪 PASS`** : **AC-4 « Nominal »** *(« le nombre est accompagné de la description en soutien discret » → **restreint aux tuiles actives**)* et **AC-3 « Erreur »**. **Mesuré** : `git diff --numstat` rend **49 insertions, 0 suppression** ⇒ encadrés **purement additifs**, ⛔ **aucune ligne d'origine réécrite**. ⚠️ **Prise d'effet DIFFÉRÉE à l'implémentation** *(aujourd'hui le code affiche encore la description, et le Story File d'US-01.1 le dit)*. ⛔ **À ce moment-là, les colonnes audit et QA d'US-01.1 passent en re-audit et ses visas devront être RAFRAÎCHIS** — application directe de **NB-6**.
- 🔴 **UN TEST VERT D'UN JOB REQUIS TOMBERA, et c'est su d'avance** : `test/e2e/hub_echeances_test.dart` boucle sur le jeu d'exemple et assère `find.text(<description>)` **dans chaque tuile par sa clé**, en `findsOneWidget` — assertion **posée par la QA** pour tuer les mutants **X-2** et **X-3**. ⛔ **Elle se BORNE, elle ne se supprime pas** *(la retirer les ressuscite)*, et le pas `.feature` d'US-01.1 doit bouger **dans le MÊME commit** sous peine d'incohérence de `check_gherkin_mapping.py`.
- 🔬 **DÉFAUT D'INSTRUMENT RELEVÉ PAR MESURE PENDANT CE RITUEL, à porter à `/audit-methodo`** : les couples `(.feature, fichier de tests)` de `scripts/check_gherkin_mapping.py` sont une **liste EN DUR**, et ⛔ **aucun contrôle ne vérifie qu'un `.feature` du dossier y figure**. **MESURÉ LE 2026-08-21** *(observation datée, ⛔ pas une valeur vivante — le total du dépôt bouge à chaque scénario ajouté, et il a bougé dès le lendemain)* : **203** scénarios sur disque, **63** vus par le gate, **140 invisibles** — dont **103** d'exclusion **assumée** (gouvernance), le reste étant ceux d'US-01.4, invisibles **sans que rien ne le signale**. **Pour re-mesurer** : comparer la somme des scénarios de `tests/features/*.feature` à ce que `check_gherkin_mapping.py` **imprime**. **Même famille que le défaut ⑥** : les instruments vérifient la cohérence de ce qui est là, **jamais la complétude de ce qui devrait y être**.
- ⛔ **CE QUE CE VISA N'ATTESTE PAS** : **0 ligne de `lib/`** · **aucun écran vu** · les **37 scénarios ne sont PAS exécutés** et ⛔ **pas encore sous le contrôle de correspondance** *(le couple n'est pas inscrit dans `COUPLES` — l'y mettre maintenant rendrait le job requis ROUGE, `FICHIER DE TESTS ABSENT`, lu dans le code du script)* · la migration **`v2 → v3`** de `ÉCHUE RETIRÉE` **n'existe pas** · **aucun ADR** écrit · **NM-3, NM-11, NM-12, NM-13** non levées.
- ⚖️ **AC-11 AJOUTÉ LE 2026-08-22, par décision humaine — et il comble une LACUNE, il n'élargit pas le périmètre.** *Un retrait dont l'écriture échoue* n'avait **ni AC, ni texte, ni surface** : **AC-17 d'US-01.2 énumère TROIS actes** *(création, édition, suppression)* et le retrait est un **quatrième** ; `ActeEcriture` ne porte que **deux** textes alors que son propre motif exige *« l'utilisateur doit savoir **ce qui** n'a pas eu lieu »* *(bloquant **NB-B**)* ; et ⛔ **le hub n'a AUCUNE surface de message** *(`MessageValidation` n'est monté que par la page de gestion — mesuré)*. **Trouvé par @Architect au gate *analyze*, pas par un instrument.** ⚠️ **Il arrive APRÈS `EVT_STORY_READY`** et ⛔ **aucun événement du catalogue ne modélise un changement de périmètre** — ⛔ **aucun n'a été détourné**. 🔴 **Conflit d'ordinal NOMMÉ et NON tranché** : `CLAUDE.md` attribue la **4ᵉ instance** aux AC-16/AC-17 d'US-01.2, et **R-17 de ce Story File attribue la 4ᵉ à la fusion d'US-01.4** ⇒ **deux occurrences portent le même rang**. ⛔ **Rien n'est renuméroté** : un ordinal recopié à la main dans plusieurs documents est la **classe de défaut nº 1**, et *« le rang n'est pas l'information — la récurrence l'est »*.
- 🔬 **DEUXIÈME DÉFAUT D'INSTRUMENT, DÉCOUVERT EN ÉCRIVANT AC-11 — et il frappe le SEUL contrôle du projet capable de voir une clause MANQUANTE.** @ProductOwner a d'abord rédigé la clause « Erreur » d'AC-11 dans le **style exact d'AC-17 d'US-01.2** *(`- **Erreur** — **la clause qui porte toute la valeur** :`)* : le compteur a rendu **un de moins** que `AC × 3` — **la clause était INVISIBLE au motif**, le tiret s'intercalant avant le deux-points. ⇒ ⛔ **Une variante de prose, même COPIÉE D'UNE US CERTIFIÉE, rend le contrôle aveugle sans rien casser d'autre.** Remède identifié et **inexistant** : un script comparant `AC × 3` au compte de clauses et **refusant l'écart**, avec son autotest de mutation. ➡️ **`/audit-methodo`.**
- ✅ **VALIDATION TECHNIQUE — 2026-08-22. `EVT_ARCHI_VALIDATED` émis, phase → `parallel_design`.** **ADR-012** *(état `ÉCHUE RETIRÉE` persisté, migration `v2 → v3`)* et **ADR-013** *(état d'interaction de la grille, observabilité et accessibilité des gestes)* **ACCEPTÉS** — donc **IMMUABLES** — après relecture de leurs §Décision **et REJEU des sondes par @Architect lui-même** *(`flutter test` ⇒ **10 tests passés**, mutant `SONDE-5b` **TUÉ**)*, ⛔ **pas sur la foi d'un rapport**. **Trois PROPRIÉTÉS plutôt que des tests de garde** : clé `retiree` absente = présente ⇒ **AC-5 « Erreur » vrai PAR CONSTRUCTION** · **exclusivité STRUCTURELLE** *(un champ nullable ne peut pas contenir deux `id`)* · **`compareTo` INCHANGÉ** *(sinon AC-6 « Erreur » d'US-01.1 changerait de sens sans qu'aucun AC ne le demande)*. Le `Remplace` d'ADR-012 est **borné à UNE puce d'ADR-010 §2** : l'interdiction **I-7** est **réduite d'un champ, nommément** — `retireeLe`, `createdAt`, `dirty`, `version` **restent interdits mot pour mot**, et ⛔ **ADR-010 n'est PAS édité** *(précédent ADR-007 sur ADR-006)*.
- 🔬 **DEUX CONSTATS DE @ARCHITECT DU 2026-08-21 SONT RÉFUTÉS PAR LA MESURE — et c'est @Architect qui les a réfutés, en construisant des sondes.** **①** *« le scénario du rafraîchissement pendant la révélation est inobservable sans seam »* est **FAUX** : la technique est le **PHASAGE de l'appui** *(`SONDE-5` : appui à `t=28 s`, tic à `t=30 s`, **révélation toujours visible**, retour du nombre à `t=32 s` ; mutant `SONDE-5b` **TUÉ**)* ⇒ ⛔ **ADR-013 n'introduit AUCUN seam** et **ADR-011 §5 en sort RENFORCÉ**. **②** La **lettre** du constat `C-3` rendait **AC-9 INSATISFIABLE** : `SONDE-1` montre qu'un `onDoubleTap` **seul** rend la tuile **FOCALISABLE ET INERTE** — clavier **0 appel**, AT **0 appel**, **`erreur=aucune`** — soit exactement le **« mensonge d'interface »** qu'AC-9 « Erreur » interdit, et ⛔ **aucun scénario fonctionnel ne l'aurait vu**. L'arbre retenu est **mesuré sur QUATRE canaux** *(pointeur simple **0**, double **1**, ENTRÉE **2**, ESPACE **3**, AT **4**, **un seul** nœud portant le libellé)*.
- 🔴 **QUATRE AFFIRMATIONS FAUSSES DE @ARCHITECT CORRIGÉES DANS LE STORY FILE — dont UN CONTRÔLE PRESCRIT QUI ÉTAIT FAUX, ce qui est plus grave qu'une phrase fausse.** Il prescrivait `gh pr list` pour décider si US-01.2 est fusionnée : **sans `--state`, cette commande ne voit que les PR OUVERTES** ⇒ il aurait rendu « non fusionnée » **INDÉFINIMENT** et **bloqué le développement sur un verdict inexistant**. **Mesuré** : `gh pr view 32` rend **`MERGED`**, fusionnée le **2026-08-19T08:26:44Z** par **`gitgdx`** ⇒ **US-01.2 EST sur `main`**, ⛔ **rien ne bloque le développement**. ✅ **Contrôle juste, qui mesure l'ÉTAT et non la paperasse** : `git merge-base --is-ancestor feat/US-01.2-design origin/main` → **exit 0**. ⚖️ **Le PROJECT_LOG du 2026-08-18 avait RAISON et il a été accusé à tort** *(la QA avait mesuré avec `--state all` → `[]`, **exact à sa date** : elle a tourné **avant** la création de la PR à 15:35:57Z)*. ⚠️ **Le contrôle négatif de cette correction est lui-même LEXICAL et rend 4 faux positifs** — les 4 occurrences restantes sont des **citations dans leur réfutation** *(vérifié ligne par ligne)* : ⛔ **il n'y a pas de troisième voie lexicale**, c'est [`tension_structurelle.md`](reports/US-00.5/tension_structurelle.md) en direct.
- 🔒 **INTEGRATION LOCK — 2026-08-24. `EVT_DESIGN_COMPLETED` émis, phase → `development_start`, @Developer est déverrouillé.** **Visas `@Data` et `@UX` posés.** **ADR-014** *(enveloppe interactive **conditionnelle** ; où vit l'état du message du hub)* **ACCEPTÉ**, donc immuable, après relecture **et vérification indépendante de ses fondements** par @Architect.
- 🔬 **LA JOINTURE A TROUVÉ SIX TROUS ET REFERMÉ UN POINT OUVERT — et c'est le DEUXIÈME relevé consécutif, pas une opinion.** US-01.2 en avait trouvé **quatre** avec deux branches **en aveugle** ; cette fois **chacune savait ce que faisait l'autre**, et il en restait **six**. **T3** contredisait **ADR-012 §4** *(c'est le mutant `M2`, qui rougit `B2 B3 B5`)* → rectifiée · **J-1** l'arbre d'ADR-013 était publié **sans condition** → **ADR-014 A** · **J-2** l'état du message → **ADR-014 B** · **D-4** `retiree: null` ⇒ résidu, **absent du corpus** → arbitré voie (b) et **écrit** · **D-5** position de la clé → contrôle de **revue** · **D-8** historique **sans plafond** → *« mesurer d'abord, borner ensuite »* avec son **instrument livré**.
- ✅ **D-7 est le cas qui montre À QUOI SERT cette étape.** @UXDesigner a décidé **« rien à afficher »** — mais **sous une borne NOMMÉE** *(« tant que D-1 tient »)*, en disant **pourquoi elle compte** *(sinon **une tuile disparaîtrait sans geste** et AC-5 « Erreur » serait contredit **par le chemin d'écriture**)*. @DataEngineer a **confirmé la borne** : le domaine de `retiree` est **fermé par le type**. ⇒ 🎓 **Acquis de méthode : une décision de SILENCE est recevable quand elle est posée sous une borne que l'autre branche peut CONFIRMER OU RÉFUTER.** C'est l'inverse exact du trou d'US-01.2, où ⛔ **personne n'avait posé la question.**
- 🔴 **TROIS AFFIRMATIONS DE @ARCHITECT CORRIGÉES, dont deux par les agents eux-mêmes.** **①** Mon brief affirmait *« un SM T580 est branché »* : `adb devices` rend une **LISTE VIDE** — j'ai transporté une **mesure de trois jours** comme un **fait présent**, **classe de défaut nº 1 dans sa forme la plus pure**. **②** J'avais fondé **J-2** sur *« ADR-013 exige un ADR pour un quatrième état éphémère **dans ce `State`** »* : le seuil **n'est PAS franchi**, la décision B gardant la grille à **trois** états. L'ADR reste justifié, mais sur un fondement **autre et plus faible** — il **retient une alternative qu'ADR-013 a écartée** *(L297)* et **écarte le remède qu'ADR-013 désignait** *(L369)*, **les deux vérifiés par moi**. **③** J'avais qualifié **J-1** d'*« exactement `SONDE-1` »* : c'est son **MIROIR** *(annoncé sans effet, au lieu d'actionnable non annoncé)*, et ⛔ **la distinction change l'assertion à écrire**.
- 🔬 **DEUX INSTRUMENTS LIVRÉS, TOUS DEUX REJOUÉS PAR MOI.** `migration_v3_guard_criterion.py` : **`--selftest` exit 0** *(8 assertions, **toutes** tuées)*, **`--croise` exit 0**, **défaut exit 1**. `cout_ecriture_atomique_criterion.py` : **`--selftest` exit 0** *(6 vérifications, **toutes** tuées)*, **défaut exit 1**, **`--appareil` exit 1**. 🔴 **La matrice croisée est le résultat le plus lourd du cycle** : **QUATRE mutants destructeurs passent le critère d'aller-retour d'US-01.2 avec 8 assertions sur 8 VERTES**, et le mutant `M6` **n'est vu que par ce critère seul** ⇒ **les deux instruments ne sont pas redondants DANS LES DEUX SENS**. Et **le mutant le plus dangereux est celui qui a l'air le plus propre** *(« redescendre proprement en retirant la clé » — il ferait tomber **AC-12 « Erreur » d'US-01.2**, une US **déjà validée**)*.
- 🆕 **FAIT NEUF SUR D-8, MESURÉ, qui en déplace le risque** : le coût de l'écriture atomique est **dominé par un terme FIXE** *(flush + rename)*. Taille **×1209** ⇒ durée **×1,39 à ×2,71** seulement, et à `n=100` / `n=1000` la médiane est passée **SOUS** celle de `n=0`. ⇒ **le risque que D-8 nomme porte sur la TAILLE, pas encore sur la LATENCE.** L'estimation de la branche data *(« ≈140 o »)* est confirmée **et désormais LUE** : **141,0 octets exactement**, **1 411 167 octets** à 10 000.
- ⛔ **CE QUE CE LOCK N'ATTESTE PAS** : **0 ligne de `lib/`** · **aucun écran vu** · les **scénarios ne sont ni exécutés ni sous le contrôle de correspondance** *(couple non inscrit dans `COUPLES` — c'est **T15**)* · **aucune ligne de `v3` n'existe** · 🔴 **la mesure APPAREIL de D-8 est SPÉCIFIÉE ET NON EXÉCUTÉE** *(`adb devices` **vide** ⇒ `--appareil` rend **exit 1** en le disant)* · **la croissance de l'historique reste NON BORNÉE**, assumée et datée · **NM-3, NM-11, NM-12, NM-13** non levées, et ⚠️ **NM-13 ne sera que PARTIELLEMENT levable sur un SM T580** *(ses tuiles font plusieurs centaines de dp ; le cas critique est un **petit téléphone à 320 dp**)*.
- 📌 **`pattern 14` ne résout NULLE PART** — renvoi mort **cité par ADR-013, un ADR ACCEPTÉ**, et deux fois par le Design UX. `STACK_PROFILE.md` ne porte **aucune table numérotée de patterns** *(vérifié)*. ⛔ ADR-013 n'est pas édité. ➡️ **`/audit-methodo`.**
- **Reste dû** : §Contexte technique, §Tâches d'implémentation et partie technique des §Critères de test *(@Architect)*, puis `EVT_ARCHI_VALIDATED`, puis **`parallel_design`** — **Design Data ET Design UX, ⛔ aucun `N/A` justifiable en track FULL** *(ADR-008 : la clause n'est pas assouplie)*. **10 tâches de passation P-1 → P-10** sont inscrites dans le Story File par @ProductOwner.
- **Prochaine étape** : @Architect — sections techniques, ADR de la représentation persistante de `ÉCHUE RETIRÉE`, puis Integration Lock. ⚠️ **Le développement ne peut pas commencer avant la fusion d'US-01.2** *(dépendance de persistance, et `strict: true` sérialise les merges)*.
