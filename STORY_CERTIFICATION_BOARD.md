...
- ✅ **@PO Visa — 2026-08-21.** Story File **fonctionnel** *(AC, clauses, Gherkin, table anti-orphelin, critères de test fonctionnels, DoD)* + `tests/features/US-01.4-gestes-tuile.feature`. **Comptés PAR COMMANDE, jamais écrits à la main** *(défaut ⑤)* : `grep -c "^  Scénario: "` → **37** · `grep -c "^### AC-"` → **10** · clauses Nominal/Erreur/Limite → **30** · doublons de titres → **sortie VIDE** · collision de titres avec US-01.1 et US-01.2 → **sortie VIDE**. **Deux recoupements indépendants tiennent** : `10 × 3 = 30` ⇒ **aucun AC amputé d'une clause** *(seul contrôle capable de voir une clause **manquante** — la table anti-orphelin, elle, ne verrait que les clauses **écrites**)*, et la somme des scénarios par AC = **37** = le compte du `.feature`.
...
- 🔬 **DÉFAUT D'INSTRUMENT RELEVÉ PAR MESURE PENDANT CE RITUEL, à porter à `/audit-methodo`** : les couples `(.feature, fichier de tests)` de `scripts/check_gherkin_mapping.py` sont une **liste EN DUR**, et ⛔ **aucun contrôle ne vérifie qu'un `.feature` du dossier y figure**. **Mesuré** : **203** scénarios sur disque, **63** vus par le gate, **140 invisibles** — dont **103** sont l'exclusion **assumée** de la gouvernance et **37** sont ceux d'US-01.4, invisibles **sans que rien ne le signale**. **Même famille que le défaut ⑥** : les instruments vérifient la cohérence de ce qui est là, **jamais la complétude de ce qui devrait y être**.
