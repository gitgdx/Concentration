...
| US-00.5 (ex US-INIT-05) | **ADR-001 (choix de stack) + exactitude de l'Art. 4 de la Constitution** | [US-00.5-adr-stack-constitution.md](../stories/US-00.5-adr-stack-constitution.md) | 🔨 **en cours** — créée le **2026-07-30** via `/us-new`, track **STANDARD**, phase `business_alignment`. **Périmètre `SOCLE SEUL`** arbitré par l'humain : **(1)** ADR-001, **(2)** amendement de l'Art. 4. ⚠️ **PÉRIMÉ-2026-07-30 — la mention antérieure « périmètre RÉDUIT : la correction de la règle 2 et de l'Art. 4 est SANS OBJET » n'était vraie qu'à MOITIÉ** : la **règle 2** de `CLAUDE.md` et la phrase « *requis par la protection de branche* » sont bien **devenues vraies** *(correction sans objet, et les « corriger » serait une régression documentaire)*, **mais l'Art. 4 est FAUX sur trois autres points**, établis **par exécution** le 2026-07-30 — il annonce un **SAST bloquant** *(`run_gates --gate sast` → « aucun gate ne correspond »)*, un **audit de dépendances bloquant** *(`"blocking": false`)* et un seuil **`coverage_ratchet`** *(absent de `factory.config.json`)*, et **omet** le gate réel `app.build` |
...
- [ ] Toutes les US listées (US-INIT, US-00.1→US-00.6, **US-00.7**) sont `Certifié Prod = 🚀 OUI` (ou clôturées/justifiées dans le SCB) — *(**PÉRIMÉ-2026-07-30** : l'état antérieur disait « **4 sur 7**, US-00.7 en cours »)*. **État au 2026-07-30 : 5 sur 7** — US-00.1, US-00.2, US-00.3, US-00.4 et **US-00.7 🚀**. **Restent US-00.5** *(🔨 en cours depuis le 2026-07-30)*, **US-00.6** *(à créer)*, et le **statut d'`US-INIT` à clarifier*
